...
from app.tools.qfw_compat import apply_patches; apply_patches()
import sentry_sdk
from sentry_sdk.integrations.loguru import LoguruIntegration, LoggingLevels
from PySide2.QtCore import Qt, QTimer, QEvent, qInstallMessageHandler
...
def initialize_application():
    ...
    Returns:
        tuple: (program_dir, semaphore, is_first_instance)
    """
    ...
    semaphore, is_first_instance = check_single_instance()

    return program_dir, semaphore, is_first_instance


def handle_existing_instance(semaphore):
    """处理已存在的应用程序实例
...
def cleanup_resources(
    semaphore, local_server, url_handler, cs_ipc_handler, update_check_thread
):
    """清理应用程序资源
...
def main():
    ...
    program_dir, semaphore, is_first_instance = initialize_application()

    if not is_first_instance:
        handle_existing_instance(semaphore)
