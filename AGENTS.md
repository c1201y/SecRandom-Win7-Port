...
- Windows 7 desktop startup forces Avalonia software rendering with redirection-surface composition, and `DrawAudioService` skips the bundled MiniAudio/WASAPI backend on Windows 7 or non-x64 Windows. These native compatibility guards are intentional; draws remain usable without music on affected systems.
