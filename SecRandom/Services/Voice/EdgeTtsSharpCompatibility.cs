...
namespace Edge_tts_sharp
{
    ...
    // net6 adaptation: ClientWebSocket has no ConnectAsync(HttpMessageInvoker) overload,
    // so handshake headers are applied through Options.SetRequestHeader instead of HttpClient.
    public sealed class Wss : IDisposable
    {
        ...
        public bool Run()
        {
            ...
            try
            {
                _socket.ConnectAsync(_uri, connectCancellation.Token).GetAwaiter().GetResult();
                _ = ReceiveAsync();
        ...
        private void ApplyHandshakeHeaders()
        {
            // The service rejects the bare .NET handshake; mirror the browser-like
            // headers the HttpClient-based transport used to send.
            ...
            var muid = new byte[16];
            RandomNumberGenerator.Fill(muid);
            options.SetRequestHeader("Cookie", $"muid={Convert.ToHexString(muid)};");
        }
