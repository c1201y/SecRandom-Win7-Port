...
_MAX_CACHED_HISTORY_FILES = 20


def _evict_cache_if_needed():
    if len(_history_data_cache) > _MAX_CACHED_HISTORY_FILES:
        # 移除最早插入的(耗时最久的)条目
        excess = len(_history_data_cache) - _MAX_CACHED_HISTORY_FILES
        for _ in range(excess):
            try:
                _history_data_cache.pop(next(iter(_history_data_cache)))
