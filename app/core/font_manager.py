...
def _apply_system_font(font_family: str, font_weight: int) -> str:
    ...
    logger.debug(f"已应用字体: {font_family} (粗细值: {font_weight})")
    return font_family
...
def apply_font_to_application(font_family: str) -> None:
    ...
    for widget in QApplication.allWidgets():
        if isinstance(widget, QWidget) and widget.font().family() != font_family:
            wf = QFont(app_font)
            cf = widget.font()
            wf.setBold(cf.bold())
            wf.setItalic(cf.italic())
            widget.setFont(wf)
            updated += 1
    logger.debug(f"已应用字体: {font_family}, 更新了{updated}个控件")
