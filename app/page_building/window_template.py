...
# ==================================================
# 修复FluentTitleBar图标在高DPI下模糊的问题
# ==================================================
_FluentTitleBar = FluentTitleBar
_original_set_icon = _FluentTitleBar.setIcon


def _patched_set_icon(self, icon):
    try:
        if hasattr(self, 'iconLabel') and self.iconLabel is not None:
            dpr = self.iconLabel.devicePixelRatioF()
            target_size = max(1, int(18 * dpr))
            pix = QIcon(icon).pixmap(target_size, target_size)
            pix.setDevicePixelRatio(dpr)
            self.iconLabel.setPixmap(pix)
            return
    except Exception:
        pass
    _original_set_icon(self, icon)
_FluentTitleBar.setIcon = _patched_set_icon
# ==================================================
...
class SimpleWindowTemplate(FramelessWindow):
        ...
        try:
            ...
            try:
                if icon and not icon.isNull():
                    dpr = self._sr_title_icon_label.devicePixelRatioF()
                    target_size = max(1, int(16 * dpr))
    ...
    def initWindow(self, title: str, width: int = 700, height: int = 500) -> None:
        ...
        window_icon = QIcon(
            str(get_data_path("assets/icon", "secrandom-icon-paper.ico"))
        )
