...
from PySide2.QtWidgets import QHBoxLayout, QVBoxLayout, QWidget, QSizePolicy
...
class StudentTagSettingWindow(QWidget):
    ...
    def _init_ui(self) -> None:
        self.setWindowTitle(get_content_name_async("tag_setting", "title"))

        self.main_layout = QVBoxLayout(self)
        self.main_layout.setContentsMargins(20, 20, 20, 20)
        self.main_layout.setSpacing(15)

        self.title_label = TitleLabel(get_content_name_async("tag_setting", "title"))
        self.main_layout.addWidget(self.title_label)

        self.description_label = BodyLabel(
            get_content_name_async("tag_setting", "description")
        )
        ...
        self.table.setHorizontalHeaderLabels(
            get_content_name_async("tag_setting", "header_labels")
        )
        ...
        self.save_button = PrimaryPushButton(
            get_content_name_async("tag_setting", "save_button")
        )
    ...
    def _get_class_name(self) -> str:
        ...
        return str(
            readme_settings_async("roll_call_list", "select_class_name") or ""
        ).strip()

    def _get_list_file(self):
        class_name = self._get_class_name()
        return get_data_path("list", "roll_call_list") / f"{class_name}.json"
        ...
        try:
            ...
            if not class_name:
                config = NotificationConfig(
                    title=get_content_name_async("tag_setting", "error_title"),
                    content=get_content_name_async("tag_setting", "no_list_selected"),
                    duration=3000,
                )
            ...
            if not list_file.exists():
                config = NotificationConfig(
                    title=get_content_name_async("tag_setting", "error_title"),
                    content=get_content_name_async("tag_setting", "list_file_missing"),
                    duration=3000,
                )
            ...
            config = NotificationConfig(
                title=get_content_name_async("tag_setting", "success_title"),
                content=get_content_name_async("tag_setting", "success_message"),
                duration=3000,
            )
            ...
            config = NotificationConfig(
                title=get_content_name_async("tag_setting", "error_title"),
                content=f"{get_content_name_async('tag_setting', 'save_error')}: {str(e)}",
                duration=3000,
            )
            show_notification(NotificationType.ERROR, config, parent=self)
            logger.exception(f"保存学生标签失败: {e}")

    def closeEvent(self, event):
        if not self.saved:
            dialog = Dialog(
                get_content_name_async("tag_setting", "unsaved_changes_title"),
                get_content_name_async("tag_setting", "unsaved_changes_message"),
                self,
            )

            dialog.yesButton.setText(
                get_content_name_async("tag_setting", "discard_button")
            )
            dialog.cancelButton.setText(
                get_content_name_async("tag_setting", "continue_editing_button")
            )
