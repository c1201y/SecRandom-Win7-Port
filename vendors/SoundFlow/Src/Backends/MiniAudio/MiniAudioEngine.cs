...
public sealed class MiniAudioEngine : AudioEngine
{
    ...
    public override void UpdateDevicesInfo()
    {
        ...
        {
            if (pPlaybackDevices != IntPtr.Zero) Native.FreeDeviceInfos(pPlaybackDevices, playbackCountUint);
            if (pCaptureDevices != IntPtr.Zero) Native.FreeDeviceInfos(pCaptureDevices, captureCountUint);
        }
    }
}