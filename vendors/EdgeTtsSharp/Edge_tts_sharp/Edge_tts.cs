...
public class EdgeTts
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        wss.OnMessage += (sender, e) =>
        {
            if (e.IsText)
            {
                ...
                else if (data.Contains("Path:turn.end"))
                {
                    ...
                    {
                        CompleteError(new Exception("返回值为空！"));
                    }
            ...
            {
                var data = e.RawData;
                // Edge TTS binary frames start with a big-endian two-byte
                // header length. The old parser searched the entire frame as
                // UTF-8, which can lose the audio boundary on newer servers.
                if (data.Length < 2)
                    return;
                ...
                if (!header.Contains("Path:audio", StringComparison.OrdinalIgnoreCase) || payloadLength == 0)
                    return;

                var curVal = data.Skip(payloadOffset).ToArray();
        ...
        wss.OnColse += (sender, e) =>
        {
            ...
            CompleteError(new InvalidOperationException("Edge TTS 连接在返回音频前关闭。"));
        };
        wss.OnLog += (onmsg) =>
        {
            if(Debug) Console.WriteLine($"[{onmsg.level.ToString()}] {onmsg.msg}");
            if (onmsg.level == level.error)
                CompleteError(new InvalidOperationException(onmsg.msg));
        };
        if (wss.Run())
        {
            // Keep the service's current stable MP3 format. The RIFF PCM format
            // is not consistently accepted by the current Edge endpoint.
        ...
        {
            CompleteError(new InvalidOperationException("无法连接到 Edge TTS 服务。"));
        }
        while (Await && Volatile.Read(ref isCompleted) == 0)
        {
            Thread.Sleep(10);
        }
    }
